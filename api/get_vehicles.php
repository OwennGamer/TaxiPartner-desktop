...
try {
    $stmt = $pdo->query("SELECT p.id, p.rejestracja, p.marka, p.model, p.przebieg, p.ubezpieczenie_do, p.przeglad_do, p.aktywny, p.inpost, p.taxi, p.taksometr, p.legalizacja_taksometru_do, p.gaz, p.homologacja_lpg_do, p.firma, p.forma_wlasnosci, p.numer_polisy, p.wymiana_oleju_data, p.wymiana_oleju_przebieg, (SELECT MAX(data_dodania) FROM inwentaryzacje i WHERE i.rejestracja = p.rejestracja) AS ostatnia_inwentaryzacja FROM pojazdy p");
