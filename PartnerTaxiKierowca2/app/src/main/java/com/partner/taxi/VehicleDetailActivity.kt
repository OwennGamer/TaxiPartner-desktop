...
class VehicleDetailActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        vehicle?.let { v ->
            ...
            findViewById<TextView>(R.id.tvAktywny).text = "Aktywny: ${if (v.aktywny) "tak" else "nie"}"
            findViewById<TextView>(R.id.tvOstatniKierowca).text = "Ostatni kierowca ID: ${v.ostatni_kierowca_id ?: ""}"
            findViewById<TextView>(R.id.tvInpost).text = "Inpost: ${if (v.inpost) "tak" else "nie"}"
            findViewById<TextView>(R.id.tvTaxi).text = "Taxi: ${if (v.taxi) "tak" else "nie"}"
            findViewById<TextView>(R.id.tvTaksometr).text = "Taksometr: ${if (v.taksometr) "tak" else "nie"}"
            findViewById<TextView>(R.id.tvLegalizacjaTaksometru).text = "Legalizacja taksometru do: ${v.legalizacja_taksometru_do ?: ""}"
            findViewById<TextView>(R.id.tvGaz).text = "Gaz: ${if (v.gaz) "tak" else "nie"}"
